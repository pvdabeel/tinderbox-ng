...
def split_pn_ver(pnver: str) -> Tuple[str, str]:
    """
    Split 'name-version' into (name, version) using Gentoo PMS semantics.

    A package name may contain hyphens but no segment of it can both follow
    a hyphen *and* start with a digit. So the FIRST '-<digit>' boundary is
    canonically the name/version split point. Scanning right-to-left
    (the previous heuristic) confuses any trailing '-<integer>' BUILD_ID
    suffix that Portage prints under FEATURES=binpkg-multi-instance with
    the version's leading number, e.g. `oniguruma-6.9.10-11` was parsed as
    name='oniguruma-6.9.10' / version='11' instead of name='oniguruma' /
    version='6.9.10-11'.

    After the split we also strip a trailing '-<integer>' BUILD_ID from the
    version so emerge tokens like `oniguruma-6.9.10-r11-1` (revision r11,
    build 1) and portage-ng tokens like `oniguruma-6.9.10-r11` collapse to
    the same canonical version `6.9.10-r11`. The strip is BUILD_ID-shape
    only ('-<int>' with no 'r' prefix), so canonical Gentoo revisions
    ('-rN') survive untouched.
    """
...
def parse_emerge(path: Path) -> Tuple[Dict[str, EmergePkg], Dict[str, str]]:
    ...
    #   [binary   N    ] dev-libs/oniguruma-6.9.10-11:0/5::gentoo  USE="..." 0 KiB
    # `binary` lines are emitted when the package is being merged from a
    # pre-built binpkg (FEATURES=buildpkg consumers, e.g. tinderbox-ng's
    # session matrix). Both flavours represent "Portage will install this
    # package" and are equivalent for the set/ordering metrics; we only
    # exclude the source-vs-binary distinction from this comparison since
    # portage-ng's plan log doesn't make that distinction either.
    line_re = re.compile(r"^\[\s*(?:ebuild|binary)\s+([A-Z])[^]]*\]\s+(\S+)(?:\s+.*)?$", re.IGNORECASE)
